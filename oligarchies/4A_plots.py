...
lmic_df = df[df['group'] == 'LMIC']

# Group the data by journal and subset
grouped_df = lmic_df.groupby(['journal', 'subset']).mean().reset_index()
...
ax.set_ylabel('Percentage of LMIC Authors')
ax.set_title('LMICs: Top 10% Most Central Authors vs. All Authors')
ax.legend()
...
# Display the plot
plt.tight_layout()
plt.savefig('oligarchies/centralities/results_LMIC.png')
